...
We're not going over all the GDAL tools today. The one we're particularly interested in is `gdal_translate`. You can read its documentation here: https://gdal.org/programs/gdal_translate.html

The description says, "Converts raster data between different formats," but it can really do so much more. Here are a few examples of additional parameters you can pass the program:  
...
So I can navigate to that directory and then call `gdal_translate`. The problem is that we need to know the location of the input and output files, too. They will not be in `C:\OSGeo4W64\bin`. Let's say I have my GIS files in `C:\Users\bsmit\Documents\GIS`. How could I use `gdal_translate`?

From anywhere, I can use all absolute paths, which would work perfectly fine:

```{bash gdal 2, eval = FALSE}
C:\OSGeo4W64\bin\gdal_translate C:\Users\bsmit\Documents\GIS\my_raster.asc C:\Users\bsmit\Documents\GIS\my_raster.tif
```
...
cd C:\Users\bsmit\Documents\GIS
